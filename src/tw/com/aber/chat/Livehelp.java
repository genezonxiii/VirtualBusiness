...
public class Livehelp {
   ...
   // private static final String GUEST_PREFIX = "Guest";
    ...
    private static final Set<Livehelp> connections =
    ...
    public void incoming(String message) {
    	...
    	if(livehelper.equals(this.getName())){
    		...
    		String filteredMessage = String.format("<font class='tag'>%s</font> <b>%s</b> : %s",
            	(new SimpleDateFormat("[HH:mm]").format(new Date())),"<font color='"+"blue"+"' name='name'>"+nickname+"</font>", filter(msg[1].toString()));
            broadcast(filteredMessage, msg[0]);
    	}else{
    		String filteredMessage = String.format("<font class='tag'>%s</font> <b>%s</b> : %s",
        		(new SimpleDateFormat("[HH:mm]").format(new Date())),"<font color='"+"#e600c3"+"' name='name'>"+nickname+"</font>", filter(message.toString()));
    ...
    private static void broadcast(String msg, String withwho) {
        for (Livehelp client : connections) {
            try {
                synchronized (client) {
                	if(withwho.equals(client.getName())|| livehelper.equals(client.getName())||msg.contains("System(>_<):")){
