...
<script>
	...
	$(function() {
		...
		$("#password_btn").click(function(e) {
					...
					if ($("#pwd").val().length < 6
							|| $("#pwd").val().length > 12
							|| !regexp1.test($("#pwd").val())
							|| !regexp2.test($("#pwd").val())) {
						$("#pwd").addClass("error");
						warning_msg("密碼格式不符");
						return;
					}
				...
				<div class="form-wrap">
					<div class="form-row">
						<label for=""> <span class="block-label">新密碼</span> <input
							type="password" id="pwd" style="width: 200px;" name="password">
						</label>
					</div>
					<div class="form-row">
						<label for=""> <span class="block-label">新密碼確認</span> <input
							type="password" id="pwd2" style="width: 200px;" name="password2">
						</label>
					</div>
					<font color=brown>長度須為6~12位且包含英文及數字。</font><br>
					<br>
					<div class="btn-row">
