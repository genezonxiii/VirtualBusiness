...
function sendFileToServer(formData,status){
	sendCountTime ++;
    var uploadURL ="InvoiceImport.do"; //Upload URL
    ...
    var jqXHR=$.ajax({
		        ...
		        success: function(result){
			    	...
			    	if ((sendCountTime == sendCount) && (sendStatus == 0)){
				    	...
				    	$('#message').find("#text").val('').html("匯入完成!" + dup);
...
function createDlBtn(type){
	...
	downloadBtn.href = "./InvoiceImport.do?action=download&type=" + type
