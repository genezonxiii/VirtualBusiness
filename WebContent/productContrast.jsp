...
<head>	
	...
	$(function() {
	                ...
	                success: function(data) {
	                    ...
	                    if (resultRunTime != 0) {
	                        $.each(json_obj, function(i, item) {
	                            ...
	                            } else if (nowSelect === 'search-contrast-platform-c_product') {
	                                $('#' + nowSelect).append($("<option></option>").attr("value", json_obj[i].c_product_id_platform).text(json_obj[i].c_product_id_platform));
	                            }
	                        });
	                    }
	                }
	    ...
	    $("#search-contrast-platform-c_product").combobox();
	                        ...
	                        if (action === "insert") {
	                            $.ajax({
	                                ...
	                                data: {
	                                    ...
	                                    c_product_id_platform: $('#dialog_c_product_id_platform').val(),
	                                    amt: $("#dialog_amt").val()
	                                },
	                                success: function(data) {
	                                	console.log(data);
	    ...
	    function initGeneralEnvironment() {
	        ...
	        var conPlatCProduct_id =
	            "<td>平台用自訂商品代碼</td>" +
	            "<td>" +
	            "<input type='text' id='dialog_c_product_id_platform' name='dialog_c_product_id_platform' placeholder='輸入平台用自訂商品代碼'>" +
	            "</td>";
	        ...
	        $("#dialog-insert-table").append($('<tr></tr>').val('').html(conPlatCProduct_id));
	        ...
	        var elements = ["dialog_amt", "dialog_contrast_platform_name", "dialog_product_product_name", "dialog_c_product_id_platform", "dialog_product_product_desc", "contrast_spec_platform_name"];
	    ...
	    $("#product-contrast-table").delegate(".btn_update", "click", function(e) {
	        ...
	        var c_product_id_platform = data.c_product_id_platform;
	        ...
	        $("#dialog_c_product_id_platform").val(c_product_id_platform);
	...
	function initc_product_id_platform() {
	    $.ajax({
	        ...
	        delay: 0,
	        data: {
	            action: 'search-contrast-platform-c_product'
	        },
	        success: function(data) {
	        	var bef =$('#search-contrast-platform-c_product').val();
	            $('#search-contrast-platform-c_product')
	                .find('option')
	                .remove();
	            ...
	            var resultRunTime = 0;
	            $.each(json_obj, function(i, item) {
	                $('#search-contrast-platform-c_product').append($("<option></option>").attr("value", json_obj[i].c_product_id_platform).text(json_obj[i].c_product_id_platform));
	            });
	            $("#search-contrast-platform-c_product").find(":selected").val(bef);
	        }
	    });
